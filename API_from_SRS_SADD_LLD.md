...
1. 本文档领域边界固定为六域：寻回任务执行域、线索与时空研判域、患者档案与监护域、标签与物资运营域、AI 协同支持域、通用治理域。
2. 3.7/3.8/3.9 为同一“通用治理域”的子能力分组，不构成新增领域。
...
| 接口分组 | 所属权威域 | 说明 |
| :--- | :--- | :--- |
| 3.1 | 寻回任务执行域 | 任务生命周期与状态收敛（HC-01 唯一权威） |
| 3.2 | 线索与时空研判域 | 含匿名接入通道，不新增领域 |
| 3.3 | 患者档案与监护域 | 患者档案、监护关系管理 |
| 3.4 | 标签与物资运营域 | 标签主数据、绑定流程、工单流转 |
| 3.5 | AI 协同支持域 | 仅提供建议与会话能力，不直接改业务状态 |
| 3.6 | 查询编排层（非领域） | 跨域只读聚合视图 |
| 3.7-3.9 | 通用治理域 | 认证、用户治理、通知子能力 |
...
## 3.7 通用治理域：身份认证与基础用户接口（Auth & User）
...
## 3.8 通用治理域：管理端用户治理接口（Admin User Management）
