...
| HC-07 | 家属端必须实现 LUI（自然语言交互界面），家属可通过对话完成寻人发布、轨迹查询等核心操作。 |
...
| AI 协同支持域 | 支撑域 | AI Agent 自然语言交互、Function Calling 编排、策略建议（通过标准域 API 执行，不直接写域实体表；集成 Spring AI Alibaba） |
...
- AI Agent 通过 Function Calling 调用各域标准 REST API 执行业务操作（如发布任务、查询轨迹），所有写操作经 Policy Guard 门禁校验后由目标域服务自身完成状态变更；AI 服务不直接写入域实体表。
...
### 6.1 AI 执行边界
1. 接收用户自然语言输入并理解意图。
2. 组装上下文（任务状态、线索轨迹、患者档案）。
3. 执行检索增强（RAG）。
4. 通过 Function Calling 调用域 API 完成业务操作（如发布任务、查询轨迹）。
5. 生成策略建议并发布事件。
6. 记录审计与可观测信息。
...
**家属侧核心操作（A0-A2，支撑自然语言完成寻人发布与轨迹查询）：**

| action | 目标接口 | 执行等级 | 约束 |
| :--- | :--- | :--- | :--- |
| `create_rescue_task` | `POST /api/v1/rescue/tasks` | A2 (`CONFIRM_1`) | 家属确认后执行 |
| `query_task_snapshot` | `GET /api/v1/rescue/tasks/{task_id}/snapshot` | A0 | 只读，允许自动执行 |
| `query_trajectory` | `GET /api/v1/rescue/tasks/{task_id}/trajectory/latest` | A0 | 只读，允许自动执行 |
| `query_patient_profile` | `GET /api/v1/patients/{patient_id}` | A0 | 只读，允许自动执行 |
| `query_clue_list` | `GET /api/v1/rescue/tasks/{task_id}/clues` | A0 | 只读，允许自动执行 |
...
| `request_evidence` | `POST /api/v1/admin/clues/{clue_id}/request-evidence` | `MANUAL_ONLY` | 本版本暂不开放，仅可建议 |
...
| 影响 | 状态一致性更强；AI 具备真正的 Agent 能力（自然语言发布任务、查询轨迹），同时不破坏域权威。 |
