...
| `HC-05`（WS 路由化下发） | 客户端执行去重、防乱序与补洞，不做全量广播假设 | `TASK-03`、`NOTI-01` | `POST /api/v1/ws/ticket`、轮询补洞接口 | `ACC-FE-010` |
...
1. 调用 `POST /api/v1/ws/ticket` 获取短效一次性票据。
...
| PUB-02 | 手动录入页 | POST /api/v1/public/clues/manual-entry | 顶部说明 + short_code/captcha 表单 + 提交按钮 | 校验通过前禁用提交；429 按 Retry-After 倒计时；成功缓存匿名令牌并跳转上报页 |
...
| `captcha_token`、`device_fingerprint` | 是 | `captcha_token` 非空；设备指纹格式合法 | `E_GOV_4038`、`E_GOV_4004`、`E_GOV_4291`、`E_GOV_4292` | 429 时锁定提交按钮并显示倒计时；失败保留输入 |
