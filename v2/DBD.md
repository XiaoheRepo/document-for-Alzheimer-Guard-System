...
#### 2.6.7 user_push_token（推送令牌表）

> 支撑 API §3.8.5 推送令牌注册 / 注销；HC-05 定向下发的数据基础；HC-08 四通道中 Mobile Push 通道的寻址源。

```sql
CREATE TABLE user_push_token (
    push_token_id   bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id         bigint          NOT NULL,       -- FK逻辑: ref sys_user(id)
    platform        varchar(24)     NOT NULL,
    device_id       varchar(128)    NOT NULL,       -- 客户端稳定设备标识
...
-- user_push_token 索引
CREATE INDEX idx_push_user_active ON user_push_token (user_id, status) WHERE status = 'ACTIVE';
CREATE INDEX idx_push_platform_active ON user_push_token (platform, status, last_active_at DESC) WHERE status = 'ACTIVE';
```
